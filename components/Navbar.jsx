  ...
  return <nav className="sticky top-0 z-50 border-b border-gray-200 bg-white shadow-sm" aria-label="Primary navigation">
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      ...
      <div className="hidden min-w-0 flex-1 items-center justify-end gap-1 md:flex xl:gap-2">
        ...
        {!isAdmin && <Link href="/appointment" onClick={closeMenu} className="rounded-lg bg-sky-700 px-3 py-2 text-sm font-semibold text-white hover:bg-sky-800">{t('Book Now')}</Link>}
        {isAuthenticated && <>{(user?.role === 'admin' || user?.role === 'doctor') && <Link href="/admin" className={`rounded-lg px-2.5 py-2 text-sm ${active('/admin')}`}>{user.role === 'doctor' ? 'Doctor' : 'Admin'}</Link>}<button type="button" onClick={signOut} className="rounded-lg px-2.5 py-2 text-sm text-gray-700 hover:bg-gray-50">{t('Sign out')}</button></>}
        {/* Waitlist link temporarily disabled — preserved for future re-enablement */}
        {/* {!isAuthenticated && !isAdmin && <Link href="/waitlist" className="rounded-lg bg-sky-700 px-3 py-2 text-sm font-semibold text-white hover:bg-sky-800">{t('Join Waitlist')}</Link>} */}
      </div>
    </div>
      </div>
    {menuOpen && <div className="border-t border-gray-100 py-3 md:hidden"><div className="grid gap-1"><Link href="/" className={`rounded-lg px-3 py-2.5 text-sm ${active('/')}`} onClick={closeMenu}>{t('Home')}</Link><Link href="/about" className={`rounded-lg px-3 py-2.5 text-sm ${active('/about')}`} onClick={closeMenu}>{t('About')}</Link><Link href="/contact" className={`rounded-lg px-3 py-2.5 text-sm ${active('/contact')}`} onClick={closeMenu}>{t('Contact')}</Link>{!isAdmin && <Link href="/appointment" className="mt-2 rounded-lg bg-sky-700 px-3 py-2.5 text-center text-sm font-semibold text-white" onClick={closeMenu}>{t('Book Now')}</Link>}{isAuthenticated && <>{(user?.role === 'admin' || user?.role === 'doctor') && <Link href="/admin" className="rounded-lg px-3 py-2.5 text-sm text-gray-700 hover:bg-sky-50" onClick={closeMenu}>{user.role === 'doctor' ? 'Doctor' : 'Admin'}</Link>}<button type="button" onClick={() => { signOut(); closeMenu(); }} className="rounded-lg px-3 py-2.5 text-left text-sm text-gray-700 hover:bg-sky-50">{t('Sign out')}</button></>}
    {/* Waitlist link temporarily disabled — preserved for future re-enablement */}
    {/* {!isAuthenticated && !isAdmin && <Link href="/waitlist" className="mt-2 rounded-lg bg-sky-700 px-3 py-2.5 text-center text-sm font-semibold text-white" onClick={closeMenu}>{t('Join Waitlist')}</Link>} */}</div></div>}
  </nav>;
}
