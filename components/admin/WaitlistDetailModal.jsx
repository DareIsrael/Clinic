...
const WaitlistDetailModal = ({ entry, onClose, onStatusChange, theme }) => {
      ...
      <div className={`rounded-2xl max-w-3xl w-full max-h-[92vh] overflow-y-auto relative shadow-2xl border ${
        ...
        <div className={`px-8 py-2.5 flex items-center justify-between border-b ${
          ...
          <p className={`text-xs ${isDark ? 'text-sky-400' : 'text-sky-600'}`}>
            ...
            <span className={isDark ? 'text-sky-300' : 'text-sky-700'}>{entry.createdAt ? new Date(entry.createdAt).toLocaleDateString('en-US', { weekday: 'short', year: 'numeric', month: 'long', day: 'numeric' }) : 'N/A'}</span>
          </p>
        ...
        <div className="px-8 py-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <dl className="space-y-3.5">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <dd className={`text-sm font-medium ${isDark ? 'text-slate-200' : 'text-gray-900'}`}>{entry.firstName}</dd>
                    </div>
                    <div>
                      <dt className="text-[11px] font-medium text-gray-400 uppercase tracking-wider mb-0.5">Last Name</dt>
                      <dd className={`text-sm font-medium ${isDark ? 'text-slate-200' : 'text-gray-900'}`}>{entry.lastName}</dd>
                    </div>
                  </div>
                  <div className="flex items-start gap-2 pt-2 border-t border-gray-100/10">
                    ...
                    <div>
                      ...
                      <dd className={`text-sm font-medium ${isDark ? 'text-slate-200' : 'text-gray-900'}`}>{entry.email}</dd>
                    </div>
                  </div>
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <dd className={`text-sm font-medium ${isDark ? 'text-slate-200' : 'text-gray-900'}`}>{entry.cellPhone || 'N/A'}</dd>
                    </div>
                  </div>
                </dl>
              </div>
            ...
            <div className={`border rounded-xl overflow-hidden ${isDark ? 'border-[#334155]' : 'border-gray-200'}`}>
              <div className={`flex items-center gap-2.5 px-5 py-3 border-b ${
                ...
                <h3 className={`text-sm font-semibold ${isDark ? 'text-white' : 'text-gray-800'}`}>Address</h3>
              </div>
              <div className="px-5 py-4">
                <dl className="space-y-3.5">
                  <div>
                    <dt className="text-[11px] font-medium text-gray-400 uppercase tracking-wider mb-0.5">Street Address</dt>
                    <dd className={`text-sm font-medium ${isDark ? 'text-slate-200' : 'text-gray-900'}`}>{entry.address || 'N/A'}</dd>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <dt className="text-[11px] font-medium text-gray-400 uppercase tracking-wider mb-0.5">Country</dt>
                      <dd className={`text-sm font-medium ${isDark ? 'text-slate-200' : 'text-gray-900'}`}>{entry.country || 'N/A'}</dd>
                    </div>
                    <div>
                      <dt className="text-[11px] font-medium text-gray-400 uppercase tracking-wider mb-0.5">Postal Code</dt>
                      <dd className={`text-sm font-medium ${isDark ? 'text-slate-200' : 'text-gray-900'}`}>{entry.postalCode || 'N/A'}</dd>
                    </div>
                  </div>
                </dl>
              </div>
              ...
              <div className="px-5 py-4">
                <dl className="space-y-3.5">
                  <div>
                    ...
                    <dd className={`text-sm font-mono tracking-wide ${isDark ? 'text-slate-200' : 'text-gray-900'}`}>{entry.healthcareNumber || 'N/A'}</dd>
                  </div>
                  <div>
                    <dt className="text-[11px] font-medium text-gray-400 uppercase tracking-wider mb-0.5">Healthcare Province</dt>
                    <dd className={`text-sm ${isDark ? 'text-slate-200' : 'text-gray-900'}`}>{entry.healthcareProvince || 'N/A'}</dd>
                  </div>
                </dl>
              </div>
            </div>
          ...
          {/* ── Waitlist Record Info ── */}
          <div className={`border-t border-dashed pt-4 ${isDark ? 'border-[#334155]' : 'border-gray-200'}`}>
            <div className="flex items-center gap-2 mb-3">
              <ClipboardList className="w-3.5 h-3.5 text-gray-400" />
              <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Waitlist Record</h3>
            </div>
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className={`rounded-lg px-4 py-2.5 ${isDark ? 'bg-[#0F172A]' : 'bg-gray-50'}`}>
                ...
                <p className={`text-sm font-medium ${isDark ? 'text-slate-300' : 'text-gray-700'}`}>{entry.status || 'Active'}</p>
              </div>
            </div>
          </div>
