...
export async function POST(request) {
    try {
        ...
        // --- Resolve recipients (de-duplicated) ---
        let recipientEmails = [];

        if (targetEmail) {
            const entry = await Appointment.findOne({ email: targetEmail.trim().toLowerCase() }).lean();
            if (!entry) {
                return NextResponse.json(
                    { success: false, message: 'Email address not found in appointments' },
                    { status: 404 }
                );
            }
            ...
            const appointments = await Appointment.find(query).select('email').lean();
            recipientEmails = [...new Set(appointments.map(a => a.email.toLowerCase()))];
        }
    ...
    `;
        ...
        console.error('Appointment broadcast error:', error);
        return NextResponse.json(
...
// GET: Preview unique recipient count
export async function GET(request) {
    try {
        ...
        console.error('Appointment preview error:', error);
        return NextResponse.json(
