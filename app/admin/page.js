...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    navbar?.classList.toggle("admin-navigation-dark", theme === "dark");
    return () => navbar?.classList.remove("admin-navigation-dark");
  }, [theme]);
    ...
    async function fetchSlotCounts() {
      try {
        ...
        const response = await fetch(
          `/api/slots/admin?startDate=${todayStr}&endDate=${todayStr}`,
        );
        const data = await response.json();
        if (data.success && data.counts) {
  ...
  const today = new Date();
  const shortDateString = today.toLocaleDateString("en-US", {
    weekday: "short",
    month: "short",
    day: "numeric",
    year: "numeric",
  ...
  return (
    <ProtectedRoute requireAdmin={true}>
      <div
        className={`h-screen w-full flex overflow-hidden font-sans transition-colors duration-200 ${
          isDark ? "admin-dashboard-dark bg-[#0F172A] text-slate-100" : "admin-dashboard bg-[#F8FAFC] text-[#1E293B]"
        }`}
      >
        {/* ═══════════════════ SIDEBAR NAVIGATION ═══════════════════ */}
        <aside
          className={`w-[240px] h-full flex flex-col justify-between p-5 flex-shrink-0 hidden md:flex border-r transition-colors duration-200 ${
            isDark
        ...
        >
          <div className="space-y-6">
            ...
            <div className="flex items-center gap-2.5 px-1">
              ...
              <div className="min-w-0">
                <h1
                  className={`text-xs font-black tracking-wide leading-none ${isDark ? "text-white" : "text-[#0F172A]"}`}
                >
                  St Mary Rideau
                </h1>
                <p className="text-[9px] font-extrabold text-[#94A3B8] mt-0.5 tracking-widest uppercase">
                  Family Clinic
                </p>
              </div>
            </div>
            ...
            <nav className="space-y-0.5">
              {mainNav.map((item) => {
                const isActive =
                  ...
                  (item.id === "dashboard" && activeTab === "appointments");
              ...
              <div className="pt-3 pb-1">
                <p className="text-[8px] font-extrabold text-[#94A3B8] uppercase tracking-widest px-3">
                  Comms
                </p>
              </div>
          ...
          {/* User Profile Block */}
          <div
            className={`border-t pt-4 ${isDark ? "border-[#334155]" : "border-[#F1F5F9]"}`}
          >
            <div
            ...
            >
              <div className="flex items-center gap-2 min-w-0">
                ...
                <div className="min-w-0">
                  <p
                    className={`text-[11px] font-bold leading-none truncate ${isDark ? "text-white" : "text-[#0F172A]"}`}
                  >
                    {user?.firstName || "Israel"}
                  </p>
                  <p className="text-[9px] text-[#94A3B8] font-bold mt-0.5">
                    {user?.role === "doctor" ? "Doctor" : "Administrator"}
                  </p>
                </div>
              </div>
        ...
        <div className="flex-1 flex flex-col h-full overflow-hidden">
          {/* Top Header bar */}
          <header
            className={`h-16 border-b flex items-center justify-between px-6 lg:px-8 flex-shrink-0 transition-colors duration-200 ${
              isDark
                ? "bg-[#1E293B] border-[#334155]"
                : "bg-white border-[#E2E8F0]"
            }`}
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className={`inline-flex h-8 items-center gap-1.5 rounded-lg border px-2.5 text-xs font-semibold transition-colors ${
                  isDark
                    ? "border-blue-500 bg-[#0F172A] text-white hover:bg-blue-950"
              ...
              <button
                className={`relative p-2 border rounded-lg transition ${
                  isDark
              ...
              <div
                className={`flex items-center gap-1 border px-2.5 py-1.5 rounded-lg text-[10px] font-bold ${
                  isDark
          ...
          <main className="flex-1 overflow-y-auto p-6 lg:p-8 space-y-6 max-w-7xl w-full mx-auto">
            ...
            <div>
              <h3
                className={`text-lg font-bold ${isDark ? "text-white" : "text-[#0F172A]"}`}
              >
                Welcome, {user?.firstName || "Israel"}!
              </h3>
              <p
                className={`text-xs ${isDark ? "text-slate-400" : "text-[#64748B]"} mt-0.5 font-semibold`}
              >
                Manage clinic operations and waitlists efficiently.
              </p>
            </div>
            ...
            {activeTab === "waitlist" ? (
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                <div
                  className={`rounded-xl border p-4 flex items-center justify-between transition-colors duration-200 ${
                    isDark
                      ? "bg-[#1E293B] border-[#334155]"
                      : "bg-white border-[#E2E8F0]"
                  }`}
                >
                  <div>
                    ...
                    <p
                      className={`text-lg font-extrabold mt-0.5 ${isDark ? "text-white" : "text-[#0F172A]"}`}
                    >
                      {waitlistCounts.all}
                    </p>
                  </div>
